...
def software_ready_callback(data):
	...
	controller_cmd_str = 'rosrun {} {}'.format('rover_ga', 'bidirectional_sonar_placement_evol_controller_GA.py')
	if args.debug:
...
def waypoint_callback(msg):
	...
	percent_multiplier = 100 / number_of_waypoints
	...
	if rospy.get_param('simulation_running') is True:
		if msg.last_visited_waypoint not in waypoint_history:
			...
			percent_complete = percent_multiplier * msg.last_visited_waypoint
		
		if 	msg.last_visited_waypoint < number_of_waypoints:
			if args.debug:
				print('Distance to nextwaypoint {}'.format(msg.distance_to_next_waypoint))
			percent_complete = percent_multiplier * msg.last_visited_waypoint + abs(percent_multiplier / (max(msg.distance_to_next_waypoint-1,1)))
