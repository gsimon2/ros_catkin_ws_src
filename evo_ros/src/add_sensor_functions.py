...
def copy_base_rover_file(str_host_name):
	...
	str_rover_file = subprocess.check_output('find ~ -name {} | head -n 1'.format(str_rover_file_name),stderr=subprocess.STDOUT,shell=True) #gets the abs path from OS
...
def add_lidar_rover(str_rover_file, pos = [0,0,0.4], orient = [0,0,0]):
...
<xacro:lidar_sensor
	...
	min_range="0.06"
	max_range="20.0"
	field_of_view_horizontal="${{90*M_PI/180}}"
