...
for i=7:7
    ...
    ind = winning_inds(i,:);
    num_of_sensors = ind.NumberOfSonar;
    failed_sensor = randi(num_of_sensors);  
    ...
    for j=4:3:31
        ...
        if j == (failed_sensor * 3 + 4)
            overlay_failure_zones(ind_pos_y, ind_pos_x)
...
function c = overlay_failure_zones(pos_x, pos_y)
    ...
    % Plot high failure probability zone
    xc = pos_x;
    yc = pos_y;
    x = high_failure_zone_radius*cos(theta) + xc;
    y = high_failure_zone_radius*sin(theta) + yc;
    plot(x,y,...
        'Color',failure_color,...
        'LineWidth',line_width)
    
    % Plot medium failure probability zone
    x = medium_failure_zone_radius*cos(theta) + xc;
    y = medium_failure_zone_radius*sin(theta) + yc;
    plot(x,y,...
        'Color',failure_color,...
        'LineWidth',line_width)
    
    % Plot low failure probability zone
    x = low_failure_zone_radius*cos(theta) + xc;
    y = low_failure_zone_radius*sin(theta) + yc;
    plot(x,y,...
        'Color',failure_color,...
        'LineWidth',line_width)
