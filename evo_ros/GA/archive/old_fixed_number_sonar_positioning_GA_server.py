...
class GA(object):
	...
	def next_generation(self):
		...
		child_pop = copy.deepcopy(self.genomes)
		...
		#print('Population size: {}'.format(len(self.genomes)))
		#print('Child pop size: {}'.format(len(child_pop)))
		...
		# Clear the last generation and start filling it with members from the child population
		# The most elite from the last generation is garuanteed to survive
		self.genomes = [copy.deepcopy(self.elite_ind)]
		...
		for child in child_pop:
			...
			# If the individual is different from last generation (fitness = -1) give it a new ID and add it to the next generation
			if child['fitness'] == -1:
				child['id'] = self.child_id
				self.child_id += 1
				
				# Check to make sure the pop size isnt reached yet
				if len(self.genomes) < POP_SIZE:
					self.genomes.append(child)
			
			# If the fitness is not -1 it is a surving parent put them into the lottery pool to try to survive
			else:
				lottery_pool.append(copy.deepcopy(child))
		
				# Fill the rest of the population with the best members from the previous generation
		print('Size of child pop: {}'.format(len(child_pop)))
		print('Number of ind in pop already: {}'.format(len(self.genomes)))	
		
		# Fill rest of the next generation with winning members from the lottery_pool
		for i in range(POP_SIZE - len(self.genomes)):
			tourn = random.sample(lottery_pool,TOURNAMENT_SIZE)
			...
			winner_index = fitness_list.index(max(fitness_list))			
			self.genomes.append(copy.deepcopy(tourn[winner_index]))
		
		print('Next gen size: {}'.format(len(self.genomes)))
